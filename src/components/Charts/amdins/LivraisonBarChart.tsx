...
const LivraisonBarChart: React.FC = () => {
   ...
   const allProduits = Array.from(
    new Set(
        livraisons
            .map(livraison => livraison?.produit?.nom || "Produit Inconnu") // Valeur par défaut si `produit.nom` est absent
    )
);
    ...
    const groupedData: GroupedData = livraisons.reduce((acc: GroupedData, livraison) => {
        if (!livraison) {
            console.warn("Livraison invalide détectée :", livraison);
            return acc; // Ignore les livraisons invalides
        }
    
        const mois = livraison.createdAt 
            ? new Date(livraison.createdAt).toLocaleString('default', { month: 'short' }) 
            : "Mois Inconnu"; // Utilise "Mois Inconnu" si `createdAt` est absent
    
        const pointVente = livraison.pointVente?.nom || "Point de Vente Inconnu"; // Défaut si `pointVente.nom` est manquant
        const produit = livraison.produit?.nom || "Produit Inconnu";             // Défaut si `produit.nom` est manquant
        const quantite = livraison.quantite ?? 0;                               // Défaut à 0 si `quantite` est absente
