...
import { fetchMvtStocks, addMvtStock, updateMvtStock, deleteMvtStock, selectAllMvtStocks,fetchVendeurdMvtStocks } from '@/Redux/Admin/mvtStockSlice';
...
const LivraisonVendeur: React.FC = () => {
  ...
  useEffect(() => {
    dispatch(fetchVendeurdMvtStocks({ operation:'livraison', pointVenteId:initPs }));
  ...
  initPs && console.log('id du point de vente pour le filtre => ',initPs)
  ...
  return (
    <div style={{width:'90%'}}>
      ...
      <DataGrid
        rows={mvtStocks.filter(m=>m.pointVente._id==initPs)}
        columns={columns}
        pageSize={5}
        loading={loading}
        checkboxSelection
